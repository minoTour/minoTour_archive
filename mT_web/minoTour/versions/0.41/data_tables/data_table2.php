...
if ($login->isUserLoggedIn() == true) {
	...
	*/
	//$aColumns = array( 'basename', 'length(basecalled_template.sequence)' , 'length(basecalled_complement.sequence)','length(basecalled_2d.sequence)','read_id','channel' );
  	$aColumns = array( 'basename','Template','Template_align','Template_length','Complement','Complement_align','Complement_length','2d','2d_align','2d_length');
	...
	*/
	...
	foreach ($aColumns as $col) {
 	...
 	$sQuery = "select SQL_CALC_FOUND_ROWS basename , if (basecalled_template.basename_id is null, 'N', 'Y') as Template, if (last_align_basecalled_template_5prime.basename_id is null, 'N', 'Y') as Template_align, length(basecalled_template.sequence) as Template_length, if (basecalled_complement.basename_id is null, 'N', 'Y') as Complement, if (last_align_basecalled_complement_5prime.basename_id is null, 'N', 'Y') as Complement_align, length(basecalled_complement.sequence) as Complement_length, if (basecalled_2d.basename_id is null, 'N', 'Y') as 2d, if (last_align_basecalled_2d_5prime.basename_id is null, 'N', 'Y') as 2d_align, length(basecalled_2d.sequence) as 2d_length from config_general left join basecalled_template using (basename_id) left join last_align_basecalled_template_5prime using (basename_id) left join basecalled_complement using (basename_id) left join last_align_basecalled_complement_5prime using (basename_id) left join basecalled_2d using (basename_id) left join last_align_basecalled_2d_5prime using (basename_id) ".$sWhere." group by basename ".$sOrder.$sLimit;
