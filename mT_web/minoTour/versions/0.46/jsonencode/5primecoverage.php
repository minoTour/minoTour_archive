...
if ($login->isUserLoggedIn() == true) {
	...
	if (!$mindb_connection->connect_errno) {
		...
		if($jsonstring === false){	
			...
			#$sql_template = "SELECT alignstrand,count(*) as count,refpos FROM last_align_basecalled_template_5prime where refid = '" . $_GET['seqid'] . "' and refpos >= 0 and refpos <= 20000 group by refpos;";
			$sql_template = "SELECT alignstrand,count(*) as count,refpos FROM last_align_basecalled_template_5prime where refid = '" . $_GET['seqid'] . "' group by refpos;";
