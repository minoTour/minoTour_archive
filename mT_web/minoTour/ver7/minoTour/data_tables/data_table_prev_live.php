...
if ($login->isUserLoggedIn() == true) {
	...
	*/
	...
	$aColumns = array( 'runkey','date_time','minion','runname');

	// Indexed column (used for fast and accurate table cardinality)
	$sIndexColumn = 'date_time';
	...
	*/
	...
	if (!empty($aFilteringRules)) {
		$sWhere = " WHERE user_name = '" . $user_name .  "' and ".implode(" AND ", $aFilteringRules);
	} else {
	...
	*/
	...
	while ( $aRow = $rResult->fetch_assoc() ) {
		...
		for ( $i=0 ; $i<$iColumnCount ; $i++ ) {
			...
			} elseif ($aColumns[$i] == "date_time") {
                $row[] = gmdate("Y-m-d\ H:i:s", $aRow[$aColumns[$i]]);
