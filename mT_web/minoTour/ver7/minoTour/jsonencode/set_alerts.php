	...
	if (!$mindb_connection->connect_errno) {
		...
		foreach ($typearray as &$type) {
			...
			//echo $sqlinsert;
				...
				$sqldeleteexecute = $mindb_connection->query($sqldelete);
				//echo $sqldelete . "<br>";
			}elseif ($name === "'basenotedelete'"){
                $mission = "removed";
                $alert_index = substr($reference, 10,-1);
				$sqldelete = "delete from alerts where alert_index = " . $alert_index . ";";
				//echo $sqldelete . "<br>";
				$sqldeleteexecute = $mindb_connection->query($sqldelete);
            }elseif ($name === "'disknotify'"){
                //echo "We're in notify";
                $sqldelete = "delete from alerts where name = 'disknotify';";
				$sqldeleteexecute = $mindb_connection->query($sqldelete);
                $sqlinsert = "insert into alerts (name,twitterhandle,threshold,control,complete) values (".$name . "," . $twitterhandle. ",".$threshold.",0,0 );";
                //echo $sqlinsert;
                $sqlinsertexecute = $mindb_connection->query($sqlinsert);
            }elseif ($name === "'removedisknotice'"){
                $mission = "removed";
                $sqldelete = "delete from alerts where name = 'disknotify';";
				$sqldeleteexecute = $mindb_connection->query($sqldelete);
            }else{
