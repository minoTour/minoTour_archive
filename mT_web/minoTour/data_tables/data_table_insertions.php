...
if ($login->isUserLoggedIn() == true) {
	...
	*/
	//$aColumns = array( 'ref_id','refname','ref_seq','@var_max_val:= GREATEST(A,T,G,C,I)','CASE @var_max_val WHEN A THEN \'A\' WHEN T THEN \'T\' WHEN C THEN \'C\' WHEN G THEN \'G\' WHEN I THEN \'I\' END' ,'ref_pos','A','T','G','C','I','(A+T+G+C+I)','(((A+T+G+C+I) - GREATEST(A,T,G,C,I))/(A+T+G+C+I))','(GREATEST(A,T,G,C,I)/(A+T+G+C+I))' );
	...
	*/
	...
	if (!empty($aFilteringRules)) {
		$sWhere = " WHERE (A+T+G+C+I) >= 20 and ref_id = refid and case greatest(A,T,G,C,I) WHEN A THEN 'A'
                         WHEN T THEN 'T'
                         WHEN C THEN 'C'
                         WHEN G THEN 'G'
						 WHEN I THEN 'I'
       END = 'I' AND ".implode(" AND ", $aFilteringRules);
	} else {
		$sWhere = "WHERE (A+T+G+C+I) >= 20 and ref_id = refid and case greatest(A,T,G,C,I) WHEN A THEN 'A'
                         WHEN T THEN 'T'
                         WHEN C THEN 'C'
