...
?>
                  ...
                  <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
          			  ...
          			  <div class="panel-body">
                            <?php if ($_SESSION['currentBASE'] > 0) {?>
          					...
          					<?php if ($_SESSION['current_minup'] >= 0.37) {?>
          					...
          					<?php } ?>
                          <?php }else { echo "Currently pore data is only calculated from basecalled data. This will change in the future."; }?>
                      	  </div>
          			</div>
                          <!-- /.col-lg-12 -->
  ...
  $(document).ready(function() {
      ...
      $.getJSON('jsonencode/occupancyrate.php?prev=0&callback=?', function(data) {
          //alert("success");
          options.series = data; // <- just assign the data to the series property.
...
$.getJSON('jsonencode/passfailperporemux.php?prev=0&callback=?', function(data) {
    //alert("success");
    options.series = data; // <- just assign the data to the series property.
...
$(document).ready(function() {
    ...
    $.getJSON('jsonencode/passfailcountperporemux.php?prev=0&callback=?', function(data) {
//alert("success");
        options.series = data; // <- just assign the data to the series property.
//options.series = JSON2;
        var chart = new Highcharts.Chart(options);
