...
if ($login->isUserLoggedIn() == true) {
	...
	if ($currentcheck->num_rows == 1) {
		...
		if (password_verify($_POST['current'], $result_row->user_password_hash)) {
			...
			$update = "UPDATE users SET user_email = '" . $new . "' where user_name = '". $_SESSION['user_name'] . "';";
			$updatequery=$mindb_connection->query($update);
            $_SESSION['user_email'] = $new;
			echo "<div class='alert alert-success alert-dismissible' role='alert'><button type='button' class='close' data-dismiss='alert'><span aria-hidden='true'>&times;</span><span class='sr-only'>Close</span></button><strong>Success!</strong> Email Address Changed.</div>";
		}else{
