...
if ($login->isUserLoggedIn() == true) {
	...
	if (!$mindb_connection->connect_errno) {
		//echo "HELLO";
		$jsonjobname="precoverage_" . $_GET['seqid'];
		...
		if($jsonstring === false){	
			...
			//$table_check = "SHOW TABLES LIKE 'reference_pre_coverage_template'";
			...
			$sql_template = "SELECT ref_pos as refpos, count from reference_pre_coverage_template where ref_id = '" . $_GET['seqid'] . "' order by ref_pos;";
			$sql_complement = "SELECT ref_pos as refpos, count from reference_pre_coverage_complement where ref_id = '" . $_GET['seqid'] . "' order by ref_pos;";
			$sql_2d = "SELECT ref_pos as refpos, count from reference_pre_coverage_2d where ref_id = '" . $_GET['seqid'] . "' order by ref_pos;";
			//echo $sql_template;
			$covarray;
