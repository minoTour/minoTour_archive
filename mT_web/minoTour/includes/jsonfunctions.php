...
$checkrow = "select name,json from jsonstore where name = '" . $jobname . "' ;";
			...
			$sql_template = "select (floor(start_time/60)*60 + basecalled_template.exp_start_time) *1000 as bin_floor, count(*) as chan_count from basecalled_template inner join config_general using (basename_id) inner join tracking_id using (basename_id) group by 1 order by 1;";
			$sql_complement = "select (floor(start_time/60)*60  + basecalled_complement.exp_start_time ) * 1000 as bin_floor, count(*) as chan_count from basecalled_complement inner join config_general using (basename_id) inner join tracking_id using (basename_id) group by 1 order by 1;";
