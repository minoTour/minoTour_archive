...
while (42) {#If you have to ask the significance of 42 you shouldn't be reading computer code.
	...
	while (my $ref = $sth->fetchrow_hashref) {
		...
		if ($ref->{reflength} > 0) {
            ...
            my $aligncommand = "perl win_mT_align.pl " . $ref->{runname} ; # MS
			if ($verbose){
				print $aligncommand . "\n";
			}
			#system ($aligncommand);
            system("cmd /c start /B $aligncommand "); # MS
		}
		if ($verbose){
			print "Executed...\n";
		}
...
sub jobs {
	...
	unless ($checkingrunning) {
        ...
        my $command = $phploc . "php mT_control_scripts.php " . "dbname=$dbname jobname=$jobname reflength=$reflength prev=0 minupversion=$minupversion"; # MS
        #system($command);
        system("cmd /c start /B $command "); # MS
    } else {
    	if ($verbose){
